...
def train_BCQ(state_dim, action_dim, max_action, device, args):
    # For saving files
    setting = f"{args.dataset}_{args.seed}"
...
# Trains CBO
def train_CBO(state_dim, action_dim, max_action, device, args):
    print(f"Setting: Training BCQ Penalty V3, Env: {args.env}, Seed: {args.seed}, Coeff:{args.incoeff}")

    # For saving files
    setting = f"{args.dataset}_{args.seed}_{args.incoeff}"

    # Initialize policy
    policy = BCQ.CBO(state_dim, action_dim, max_action, device, args.dataset, args.incoeff, args.discount,
    ...
    while training_iters < args.max_timesteps:
        ...
        evaluations.append(eval_policy(policy, args.env, args.seed))
        np.save(f"./results/BCQPV3_{setting}", evaluations)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample_type", default=0)  # If 0, traditional BCQ, if 1 BCQv2

    parser.add_argument("--train_CBO", action="store_true")  # If 0, traditional BCQ, if 1 prioritized BCQ
    ...
    parser.add_argument("--reg_scale", default=0.001, type=float)
    parser.add_argument("--incoeff", default=1.0, type=float)
    parser.add_argument("--q_num", default=5, type=int)
    ...
    if args.train_behavioral:
        ...
        print(f"Setting: Training CBO Penalty, Env: {args.env}, Seed: {args.seed}")
    else:
    ...
    if args.train_CBO:
        train_CBO(state_dim, action_dim, max_action, device, args)
    else:
